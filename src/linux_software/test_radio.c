...
void print_benchmark(volatile unsigned int *periph_base)
{
	...
	unsigned int clock_cycles = stop_time-start_time;
	...
	float time_spent = clock_cycles*.000000008; // change obviously
	float throughput=bytes_transferred/clock_cycles; 
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 BLAKE THACKER - Custom Peripheral Demonstration\n\r");
    *(my_periph+RADIO_TUNER_CONTROL_REG_OFFSET) = 0; // make sure radio isn't in reset
    printf("Tuning Radio to 100kHz\n\r");
    radioTuner_tuneRadio(my_periph,100000);
    printf("Playing Tune at near 100kHz\r\n");
    play_tune(my_periph,100000);
